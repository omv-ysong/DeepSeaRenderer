...
{{0.0,1.0}, {10.0,0.96}, {20.0,0.87}, {30.0,0.75}, {35.0,0.63},
 {40.0,0.5}, {45.0,0.28}, {50.0,0.2}, {60.0,0.12}, {90.0,0.0}};    // from real measurement
...
double VSF_table[55][4] = {{0.100,  5.318e+001,  6.533e+002,  3.262e+003},
                      ...
                      {180.000,  5.019e-004,  1.030e-003,  5.686e-003}};  // ang | VSF_clear | VSF_coast | VSF_turbid, from Petzold (1972) and Light and Water, Table 3.10
...
//    double field_of_view = config["field_of_view"].as<double>();
    ...
    for(int i=0; i<num_light_; i++)
    {
        std::cerr << "Light " << i << ": position [" << lights_[i].pos().transpose() <<
                     "] direction [" << lights_[i].dir().transpose() << "]" <<
                     " spectrum_RGB: [" << lights_[i].spectrum().transpose() <<
                     "] RID_type: " << lights_[i].RID_type() << std::endl;
    }
...
void Renderer::WriteDoubleMatTo8Bit(const cv::Mat &double_mat, const std::string &write_path)
{
    cv::Mat img_new(double_mat.size(), CV_8UC3);
    //double_mat.convertTo(img_new, CV_8UC3);
...
#pragma omp parallel for
    for(int r=0; r<double_mat.rows; r++)
    {
        for(int c=0; c<double_mat.cols; c++)
        {
            ...
            if(color[0]>=255.0){color[0] = 255.0;}
            if(color[1]>=255.0){color[1] = 255.0;}
            if(color[2]>=255.0){color[2] = 255.0;}
    ...
    cv::imwrite(write_path, img_new);
}

cv::Mat Renderer::ConvertDoubleMatTo8Bit(const cv::Mat &double_mat)
{
    cv::Mat img_new(double_mat.size(), CV_8UC3);
    //double_mat.convertTo(img_new, CV_8UC3);
...
void Renderer::ConvertDoubleMatTo8Bit(const cv::Mat &double_mat, std::string &output_name)
{
//    if(double_mat.rows != img_new.rows || double_mat.cols != img_new.cols)
//    {
//        std::cerr << "input output images have different size." << std::endl;
//        system("pause");
//        exit(0);
//    }
    cv::Mat img_new(double_mat.size(), CV_8UC3);
...
//        // plane equation nxX+nyY+nz(Z-d)=0
        ...
        rotation = Eigen::AngleAxisd(rot_roll,Eigen::Vector3d::UnitX())
                * Eigen::AngleAxisd(rot_pitch,Eigen::Vector3d::UnitY())
